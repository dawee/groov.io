...
void groov_update_state(groov_event_t * event) {
  ...
  groov_io_connect_event_t io_connect_event;
  ...
  } else if (state == GROOV_STATE_HANDSHAKE_RECEIVED && groov_read_io_connect_event(event, &io_connect_event)) {
    state = GROOV_STATE_IO_CONNECTED;
  }
}
